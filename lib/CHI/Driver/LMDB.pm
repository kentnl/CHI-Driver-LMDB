...
has '_existing_root_dir' =>
  ( is => 'ro', lazy => 1, builder => '_build_existing_root_dir' );
...
has '_lmdb_max_key' =>
  ( is => 'ro', builder => '_build_lmdb_max_key', lazy => 1 );

sub _build_lmdb_env {
    my ($self) = @_;
    return LMDB::Env->new( $self->_existing_root_dir . q[],
        { map { $_ => $self->$_() } keys %env_opts } );
}
